...
def self_debug(code: str, algo: str, template: dict,
               port: int, model: str, timeout: int,
               test_result: str = "") -> str:
    """
    让 LLM 审查并修复 code。
    test_result 可以传运行报错、失败用例、Traceback 等。
    """
    return ""

def dynamic_worker(port: int, model: str, queue_in, queue_out: Queue,
                   args, concurrency: int):
    r""" 动态并发 Worker，使用 ThreadPoolExecutor 管理并发请求
    Args:
        port (int): Docker 映射端口
        model (str): 模型名称
        queue_in (Queue): 输入队列
        queue_out (Queue): 输出队列
        args (argparse.Namespace): 命令行参数
        concurrency (int): 每个实例的并发请求数
    """
    ...
    def submit_one():
        ...
        algorithms = plan_methods(item, port, model, args, template)
...
def main():
    ...
    p.add_argument("--completed-file", help="（可选）已完成任务列表(JSONL/JSON)，传入后自动过滤这些 task_id")
    ...
    try:
        ...
        for s in samples: q_in.put(s)
        total_task = len(samples) * args.repeat * args.pass_k 
        q_out = Queue()
        ...
        for pw in workers: pw.join()

        q_out.put(None); wp.join()
